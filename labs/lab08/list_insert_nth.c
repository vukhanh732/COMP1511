...
struct node *insert_nth(int n, int value, struct node *head) {
    ...
    // If the linked list is empty, make new node as head of the list and 
    // return the new
    if (head == NULL) {
    ...
    // If position is 0, append the new node at the start of the list
    } else if (n == 0) {
        struct node *new = malloc(sizeof(struct node));
        new->data = value;
        new->next = head;
    ...
    // Insert in middle
        ...
        // Loop until n - 1 postion. Make temp = head and head = head->next, at
        // n - 1, temp pointer will point to the new node and new node pointer 
        // will point to head->next.
        while (position < n && head != NULL) {
            position++;
            temp = head;
            head = head->next;
        }
